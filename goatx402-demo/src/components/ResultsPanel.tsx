...
function PlaceholderIllustration({ mode }: { mode: 'vacation' | 'transport' }) {
  if (mode === 'transport') {
    return (
      <svg viewBox="0 0 260 160" width="260" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: 'block', margin: '0 auto' }} className="animate-float">
        {/* Track */}
        ...
        {/* Train body */}
        <rect x="30" y="80" width="160" height="46" rx="6" fill="#0F172A" stroke="#0F172A" strokeWidth="2"/>
        <rect x="30" y="70" width="50" height="16" rx="4" fill="#0F172A"/>
        {/* Windows */}
        <rect x="45" y="89" width="24" height="20" rx="3" fill="#0EA5E9"/>
        <rect x="80" y="89" width="24" height="20" rx="3" fill="#0EA5E9"/>
        <rect x="115" y="89" width="24" height="20" rx="3" fill="#0EA5E9"/>
        <rect x="150" y="89" width="24" height="20" rx="3" fill="#0EA5E9"/>
        {/* Wheels */}
        <circle cx="65" cy="130" r="10" fill="#0F172A"/>
        <circle cx="65" cy="130" r="5" fill="#E2E8F0"/>
        <circle cx="155" cy="130" r="10" fill="#0F172A"/>
  ...
  return (
    <svg viewBox="0 0 260 160" width="260" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: 'block', margin: '0 auto' }} className="animate-float">
      {/* Plane body */}
      <path d="M30 90 L190 70 L220 80 L190 90 Z" fill="#0F172A"/>
      <path d="M180 72 L200 55 L210 60 L190 80 Z" fill="#FF6B35"/>
      <path d="M160 88 L175 105 L185 100 L175 84 Z" fill="#FF6B35"/>
      {/* Windows */}
      <circle cx="145" cy="78" r="6" fill="#0EA5E9"/>
      <circle cx="125" cy="79" r="6" fill="#0EA5E9"/>
      <circle cx="105" cy="80" r="6" fill="#0EA5E9"/>
      {/* Clouds */}
...
function StarRating({ rating }: { rating: number }) {
  return (
    <span className="star-rating">
      ...
      <span style={{ fontSize: '0.75rem', color: '#64748B', marginLeft: '4px', fontWeight: 700 }}>{rating}</span>
    </span>
  )
}
...
function ResultCard({ opt, rank, onBook, booking, isConnected }: {
  ...
  return (
    <div 
      className={`result-card ${isBest ? 'result-card-best' : ''} animate-slide-up`} 
      style={{ animationDelay: `${rank * 0.1}s` }}
    >
      {isBest && <div className="best-badge">⚡ BEST</div>}
      <div className="result-card-top">
        <div>
          <span className="result-rank">OPTION #{rank + 1}</span>
...
export function ResultsPanel({
  ...
  if (searching || agentPicking) {
    return (
      <div className="results-panel" style={{ alignItems: 'center', justifyContent: 'center' }}>
        <div className="agent-working animate-pop">
          <div className="agent-working-title">🤖 Assistant is working…</div>
          <div className="agent-step done">✅ Request received</div>
          <div className="agent-step active">
            <span className="spinner-dot" /> Paying Booking Agent…
          </div>
  ...
  if (options.length === 0) {
    return (
      <div className="results-panel results-empty animate-pop">
  ...
  return (
    <div className="results-panel results-filled">
      <div className="results-header animate-pop">
        <h3 className="results-title">{options.length} Options Found</h3>
        <span className="results-badge" style={{ borderColor: mode === 'vacation' ? '#FF6B35' : '#0EA5E9' }}>
          {mode === 'vacation' ? '🏖️ Vacation' : '🚆 Transport'}
        </span>
      </div>
      ...
      <div className="agent-book-banner animate-slide-up" style={{ animationDelay: '0.4s' }}>
        <div>
        ...
        <button
          ...
          style={{ boxShadow: '4px 4px 0px #0F172A' }}
        >
          ⚡ Agent Book Now
        </button>
      </div>
